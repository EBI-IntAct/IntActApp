...
public class ExpandNetworkTask extends AbstractTask implements ObservableTask {
    ...
    @Tunable(description = "Type of interactors to expand network by",
            ...
            gravity = 2.0)
    public ListSingleSelection<String> nodeTypes = new ListSingleSelection<>();
    ...
    public ExpandNetworkTask(final IntactManager manager, final CyNetwork network, CyNetworkView netView) {
        ...
        if (this.network != null) {
            nodeTypes = new ListSingleSelection<>(
    ...
    public ExpandNetworkTask(final IntactManager manager, final CyNetwork network, CyNetworkView netView, View<CyNode> nodeView) {
        this.manager = manager;
        this.network = network;
        this.netView = netView;
        this.nodeView = nodeView;
        nodeTypes = new ListSingleSelection<>(ModelUtils.getAvailableInteractionPartners(network));
    ...
    private void shiftAndLayoutGridSelectedOnly(List<CyNode> nodesToLayout) {
        ...
        Set<Double> xPos = new HashSet<>();
        Set<Double> yPos = new HashSet<>();
        Set<View<CyNode>> nodeViews = new HashSet<>();
        for (View<CyNode> nodeView : netView.getNodeViews()) {
            if (nodesToLayout.contains(nodeView.getModel())) {
                nodeViews.add(nodeView);
